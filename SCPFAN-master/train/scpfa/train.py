...
if __name__ == '__main__':
    ...
    scheduler = MultiStepLR(
        optimizer, milestones=args.decays, gamma=args.gamma)
    ...
    if args.resume is not None:
        ...
        if len(ckpt_files) != 0:
            ckpt_files = sorted(ckpt_files, key=lambda x: int(
                x.replace('.pt', '').split('_')[-1]))
        ...
        if args.log_name is None:
            experiment_name = '{}-{}-x{}-{}'.format(
                args.model, 'fp32', args.scale, timestamp)
        else:
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for iter, batch in enumerate(train_dataloader):
            ...
            if (iter + 1) % args.log_every == 0:
                ...
                print('Epoch:{}, {}/{}, loss: {:.4f}, time: {:.3f}'.format(cur_epoch,
                      cur_steps, total_steps, avg_loss, duration))
        ...
        if epoch % args.test_every == 0:
            ...
            saved_model_path = os.path.join(
                experiment_model_path, 'model_x{}_{}.pt'.format(args.scale, epoch))
